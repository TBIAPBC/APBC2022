...
def capital_check(pairs, new_pair):
    """
...
def branchandbound(solutions_list, subset_list, cost_list, data, loop_count, cost_limit):
    """
    returns all combinations of capitals which have a lower cost than the limit
    ...
    if loop_count == capital_nr/2:
        ...
        for i in range(len(solutions_list)):
            for pair in subset_list:
                if lexicographic_order(solutions_list[i], pair):
                    if capital_check(solutions_list[i], pair):
        ...
        solution, cost = branchandbound(solutions_list, subset_list, cost_list, data, loop_count, cost_limit)
        return solution, cost


def bab_optimize(solutions_list, subset_list, cost_list, data, loop_count, cost_optimum):
    """
    returns the minimum cost as optimal solution
    ...
    :param data: cost matrix
    :param loop_count: number of loops, corresponds to number of city combinations in a branch
    :return:    cost_optimum - minimum cost
    ...
    if loop_count == capital_nr/2:
                ...
                if lexicographic_order(solutions_list[i], pair):
                    if capital_check(solutions_list[i], pair):
                        ...
                        if sum < cost_optimum:
                            ...
                            new_element.append(pair)
                            if len(new_element) == capital_nr/2:
                                cost_optimum = sum
        ...
        cost_optimum = bab_optimize(solutions_list, subset_list, cost_list, data, loop_count, cost_optimum)
        return cost_optimum
...
# apply branch and bound function or optimization function
if optimize:
    cost_limit = bab_optimize(subset_list, subset_list, first_cost_list, data, loop_count, cost_limit)
    print(cost_limit)

else:
    sol_list, cost_list = branchandbound(subset_list, subset_list, first_cost_list, data, loop_count, cost_limit)
