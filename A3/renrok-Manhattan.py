...
# need to be increased for the second diagonal input file.
...
def calculateMatrix(northSouth, westEast, diagonals, doDiagonal):
    ...
    for i in range(1, D.shape[0]):
        ...
        currentValue = D[i, j]
    return D, round(currentValue, 2)


def findPath(i, j, D, northSouth, westEast, diagonals, doDiagonal):
    ...
    if D[i, j] == southStep:
        i -= 1  # one step north
        return findPath(i, j, D, northSouth, westEast, diagonals, doDiagonal) + 'S'
    elif D[i, j] == eastStep:
        j -= 1  # one step west
        return findPath(i, j, D, northSouth, westEast, diagonals, doDiagonal) + 'E'
    elif D[i, j] == diagonalStep:
        i -= 1  # one diagonal (right-left) step
        j -= 1
        return findPath(i, j, D, northSouth, westEast, diagonals, doDiagonal) + 'D'
...
def main():
    ...
    if doPrintPath:
        path = findPath(D.shape[0] - 1, D.shape[1] - 1, D, northSouth, westEast, diagonals, doDiagonal)
        print(path)
