...
from Game.gudrun_p_player_base import Player
from Game.gudrun_p_shortestpaths import AllShortestPaths
...
class FastPlayer(Player):
        ...
        try:
            ...
            for x in range(self.rememberedMap.width):
                for y in range(self.rememberedMap.height):
                    if status.map[
                        x, y].status != TileStatus.Unknown:  # map[x,y].status is an enum: Unknown, Empty, Wall, Mine
            ...
            goldInPot = list(status.goldPots.values())[
                0]  # so this is a list with only one entry (?), and the entry is the amount of gold.
            ...
            if chosenPath == []:
                ...
                if self.wasPlayerInMyWay == True:  # player was already in my was last round
                    ...
                    paths = AllShortestPaths(goldCoords, self.rememberedMap, self.player_id,
                                             withPlayersAsWalls=True)  # go around other player
                    fullPath = paths.randomShortestPathFrom(currentPosition)
            ...
            if costOfMoves >= goldInPot:
                self._debugMessage(
                    "Path on known Tiles would cost " + str(costOfMoves) + " gold, but the reward is only " + str(
                        goldInPot) + " gold. Therefore I reduce the number of moves.")
            ...
            if len(chosenPath) > 0 and len(fullPath) / len(chosenPath) > status.goldPotRemainingRounds:
                self._debugMessage("The gold will be gone in " + str(
                    status.goldPotRemainingRounds) + ", I probably won't reach it in time, so I wait instead.")
                return []
            ...
            if maxMoves < len(chosenPath):
                self._debugMessage(
                    "Path I buy for " + str(self._costOfMoves(len(chosenPath[:maxMoves]))) + " gold: " + str(
                        chosenPath[:maxMoves]) + "\n = " + str(
                        self._as_directions(currentPosition, chosenPath[:maxMoves])))
                return self._as_directions(currentPosition, chosenPath[:maxMoves])
            else:
                self._debugMessage("Path I buy for " + str(self._costOfMoves(len(chosenPath))) + " gold: " + str(
    ...
    def _saveSomeGold(self, path, fraction, status):
        newPath = list()
        i = 0
        while i < len(path) and status.gold - self._costOfMoves(
                len(newPath) + 1) >= status.gold * 0.2:  # len + 1 because then I append
...
class WaitingPlayer(Player):
    ...
    def move(self, status):
        try:
            ...
            if costOfMoves >= goldInPot:
                self._debugMessage(
                    "Path on known Tiles would cost " + str(costOfMoves) + " gold, but the reward is only " + str(
                        goldInPot) + " gold. Therefore I do not move.")
                return list()
    ...
    def _isPathClear(self, path):
        isPathClear = True
        for coords in path:
            if self.rememberedMap[
                coords].obj is None:  # empty fields can be one the path. Need to check this first because if obj is None I cannot use .is_player() on it.
                continue
                ...
                if self.rememberedMap[coords].obj.is_player(self.player_id):  # my own player is not an obstacle
                    ...
                    self._debugMessage("There is another player in my way: Player " + str(
                        self.rememberedMap[coords].obj) + " is at coordinates " + str(coords))
                    isPathClear = False
    ...
    def move(self, status):
        try:
            ...
            if self.wasPlayerInMyWay == True:
                if self.rememberedMap[fullPath[0]].obj is not None:  # empty fields can be one the path
                    if self.rememberedMap[fullPath[0]].obj.is_player():
                        if not self.rememberedMap[fullPath[0]].obj.is_player(
                                self.player_id):  # my own player is not an obstacle
                            self._debugMessage("Another robot (" + str(
                                self.rememberedMap[fullPath[0]]) + ") is in my way AGAIN. I will go around it.")
                            self.wasPlayerInMyWay = False
                            paths = AllShortestPaths(goldCoords, self.rememberedMap, self.player_id,
                                                     withPlayersAsWalls=True)
                            fullPath = paths.randomShortestPathFrom(currentPosition)
                            if fullPath == []:  # if other player are walls, it is possible that no path exists.
                                self._debugMessage("There is no path to the gold. No movement sent.")
                                return []
                            fullPath = fullPath[
                                       1:]  # (I think this is because we calculate the path from the gold to the
                            fullPath.append(goldCoords)  ## player instead of the other way round)
                            self._debugMessage(str(goldInPot) + " gold is at: " + str(goldCoords))
            ...
            if fullPathLength > status.gold - self.winMargin:
                self._debugMessage(
                    "The gold is too far away to be worth getting even if I only move one field per round. (Length of shortest path = " + str(
                        len(fullPath)) + ")")
                return []
            chosenPath = list()
            for numberOfSegments in range(1,
                                          fullPathLength + 1):  # numberOfSegments is usually actually the number of segments minus one,
                ...
                if goldInPot - totalCost >= self.winMargin:
                    if totalCost < status.gold + numberOfSegments * status.params.goldPerRound:  # check if I have enough gold for the whole path
                        ...
                        self._debugMessage("Going to gold is worth it if I split path into at least " + str(
                            numberOfSegments) + " segments. (total cost: " + str(totalCost) + " )")
                        break
                    else:
                        self._debugMessage("Not enough gold to split the path into at least " + str(
                            numberOfSegments) + " segments. (total cost would be: " + str(totalCost) + " )")
            if chosenPath == []:
                self._debugMessage("The gold is too far away to be worth getting. (Length of shortest path = " + str(
                    len(fullPath)) + ")")
                return []
            ...
            if chosenPath == []:
                ...
                if self.wasPlayerInMyWay == True:  # player was already in my was last round
                    self._debugMessage(
                        "There is another player blocking the path AGAIN. This should not happen. No movements sent.")
                    return []
