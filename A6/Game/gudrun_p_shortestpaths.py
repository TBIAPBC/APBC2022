...
class AllShortestPaths:
    def __init__(self, sink, map, playerID,
                 withPlayersAsWalls):  # (?sink seems to be the position to which I want to calc the distances?)
        self.sink = sink
        self.map = map
        ...
        if withPlayersAsWalls == False:
            ...
            wm = [[self.map[x, y].status.is_blocked() or (self.map[x, y].obj is not None and (
                        self.map[x, y].obj.is_player() and not self.map[x, y].obj.is_player(playerID))) for y in
                   range(self.height)] for x in range(self.width)]
